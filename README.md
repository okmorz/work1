...
「今月・今日あといくら使えるか」「貯金の進み具合」は収入も加味した計算式で、目標開始月からの確定済み実績と残り月数から都度動的に算出します。年間目標貯金額・月の平均収入（見込み額）はいつでも目標設定画面から変更でき、変更は次の表示から即座に反映されます（詳細は下記「収入機能と使える金額の計算」を参照）。
...
支出に加えて収入（給与・副業・臨時収入・その他）も記録できます。「使える金額」は次の式で**都度動的に**算出しており、月間の目標貯金額を固定値として保存することはありません。目標金額や見込み月収を編集すると、保存し直しや再読み込みなしに次の表示から反映されます。

- **確定済み実績貯金額** = 目標開始月から「今月より前の完了済み月」について、各月の（収入合計 − 支出合計）を合算したもの（今月自体のデータは含めない）
- **残り必要貯金額** = 年間目標貯金額 − 確定済み実績貯金額
- **残り月数** = 今月を含めて終了月までの月数
- **月間貯金目標額（今月分）** = 残り必要貯金額 ÷ 残り月数
- **今月の収入額** = 今月に記録された収入の合計。ただし今月の収入データが1件もない場合は、目標設定画面で入力する「月の平均収入（見込み額）」（`estimatedMonthlyIncome`）を暫定的に使用する。収入が1件でも記録された時点で見込み額から実績値に切り替わる（両者を混ぜて平均する、といったことはしない）
- **今月あと使える金額** = 今月の収入額 − 月間貯金目標額（今月分） − 今月の支出合計
- **今日あと使える金額** = 今月あと使える金額 ÷ 今月の残り日数
- **貯金の進み具合**（ダッシュボード）: 目標開始月からの実績貯金額（収入合計 − 支出合計、見込み額は使わない）を年間目標貯金額に対する進捗（%）として表示
- **月末フィードバック**: 対象月の実績貯金額（収入−支出）と、その月の月間貯金目標額を比較したメッセージを表示。翌月分の見出し金額も同じ式で再計算される
- **目標達成予測**（支出分析ページ）: これまでの純支出（支出−収入）のペースが続いた場合の着地予測（この式は見込み月収を使わず、実績データのペースのみで計算）

年間目標貯金額は目標設定画面からいつでも変更できます。過去の「確定済み月」の実績（収入・支出の記録そのもの）は遡って変更・再計算されず、変更後の目標は「これから先」の残り期間にのみ適用されます。

目標設定画面の下部には「データの初期化」があり、目標・支出・収入をすべて削除できます（2段階の確認あり、元に戻せません）。Supabase側の削除が先に完了してからローカルデータを消す順序になっており、途中で通信エラーが起きた場合はローカルデータを保持したままエラー表示にとどめます。
